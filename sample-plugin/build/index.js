...
const plugin = new mediarithmics_plugins_typescript_helpers_1.AdRendererBasePlugin();
plugin.addOnAdContentsListener((request, res) => {
    ...
    return Promise.all([creativeP, creativePropsP]).then((results) => {
        ...
        const quantumTagProperty = creativeProps.find((prop) => { return prop.technical_name === 'quantum_tag'; }).value.value;
        const additionalPixeltag = creativeProps.find((prop) => { return prop.technical_name === '3rd_party_pixel_tag'; }).value.value;
