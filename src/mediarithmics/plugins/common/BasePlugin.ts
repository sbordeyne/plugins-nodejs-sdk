...
export abstract class BasePlugin {
  ...
  fetchConfigurationFile(fileName: string): Promise<Buffer> {
      ...
      if (e.name === "StatusCodeError") {
        ...
        throw new Error(
          `Error while calling ${method} '${uri}' with the request body '${bodyString ||
            ""}': got a ${e.response.statusCode} ${e.response.statusMessage
            } with the response body ${JSON.stringify(
            e.response.body
          )}`
        );
      } else {
    ...
    // This method can be overridden by any subclass
    protected async onInitRequest(req: express.Request) : Promise<number> {
        return Promise.resolve(200);
    }
    private onInitRequestInternal = async (req: express.Request, res: express.Response): Promise<void>  => {
        ...
        if (req.body.authentication_token && req.body.worker_id) {
            ...
            const status = await this.onInitRequest(req);
  ...
  private initInitRoute() {
    this.app.post(
      "/v1/init",
      this.asyncMiddleware(this.onInitRequestInternal)
    );
  }
